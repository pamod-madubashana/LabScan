...
export function ActivityFeed() {
  ...
  return (
    <motion.div
      ...
      className="glass-panel p-4 h-full min-w-0 max-w-full overflow-hidden"
    >
      <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">
        Recent Activity
      </h3>
      <div className="space-y-0 custom-scrollbar min-w-0 max-w-full overflow-y-auto overflow-x-hidden max-h-[340px]">
        ...
        {activities.map((item, i) => (
          <motion.div
            ...
            className="flex items-start gap-3 py-2.5 border-b border-border/30 last:border-0 group hover:bg-accent/30 px-2 -mx-2 rounded transition-colors min-w-0 max-w-full"
          >
            <div className={cn("w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0", typeDots[item.type])} />
            <div className="min-w-0 max-w-full flex-1">
              <p className="text-xs text-foreground/90 leading-relaxed truncate max-w-full" title={item.message}>{item.message}</p>
